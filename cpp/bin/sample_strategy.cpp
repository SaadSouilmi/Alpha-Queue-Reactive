...
std::vector<GridPoint> build_grid() {
    ...
    std::vector<int32_t> max_inventories; // = {5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60};
...
int main(int argc, char* argv[]) {
    ...
    double impact_half_life = get_double(impact_cfg, "half_life_sec", 30.0);
    double impact_m = get_double(impact_cfg, "m", 4.0);
    double impact_alpha = get_double(impact_cfg, "alpha", 0.01);
    ...
    // Always load roundtrip distribution for strategy timing
    DeltaDistrib roundtrip_distrib(params_path + "/delta_distrib.csv");
    ...
    auto worker = [&]() {
        while (true) {
            ...
            if (impact_type == "time_decay") {
                impact_ptr = std::make_unique<TimeDecayImpact>(impact_half_life, impact_m);
            } else if (impact_type == "ema") {
                impact_ptr = std::make_unique<EMAImpact>(impact_alpha, impact_m);
            ...
            StrategyParams strat_params{gp.q_max, gp.max_inventory, gp.threshold};
