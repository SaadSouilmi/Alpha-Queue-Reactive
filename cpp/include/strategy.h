...
public:
    ...
    bool should_trade(double signal) const {
        if (std::abs(signal) <= params_.threshold) return false;
        if (signal > 0) {
            // Want to buy (Side::Ask): remaining capacity = max_inventory - inventory
            return (params_.max_inventory - inventory_) > 0;
        } else {
            // Want to sell (Side::Bid): remaining capacity = max_inventory + inventory
            return (params_.max_inventory + inventory_) > 0;
        }
    }
