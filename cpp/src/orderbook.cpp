...
QueueDistributions::QueueDistributions(const std::string& path) {
    ...
    // Temporary storage for raw probabilities
    ...
    while (std::getline(file, line)) {
        ...
        std::getline(ss, token, ',');  // value (skip, we use index)

        // bid_q1 to bid_q4
        for (int i = 0; i < 4; i++) {
            std::getline(ss, token, ',');
            probs[0][i].push_back(std::stod(token));
        }
        std::getline(ss, token, ',');  // skip bid_q5

        // ask_q1 to ask_q4
        for (int i = 0; i < 4; i++) {
            std::getline(ss, token, ',');
            probs[1][i].push_back(std::stod(token));
        }
        // skip ask_q5 (end of line)
    }

    // Convert to cumulative probabilities
    for (int side = 0; side < 2; side++) {
        for (int level = 0; level < 4; level++) {
            cum_probs[side][level].resize(probs[side][level].size());
            double sum = 0.0;
            for (size_t i = 0; i < probs[side][level].size(); i++) {
                sum += probs[side][level][i];
                cum_probs[side][level][i] = sum;
            }
        }
    }
}
...
void OrderBook::init(const std::vector<int32_t>& bid_prices,
                     const std::vector<int32_t>& bid_vols,
...
void OrderBook::clean_bid() {
    ...
    if (bid_.empty()) {
        // Create a new level (use a reasonable default)
        int32_t new_price = ask_.empty() ? 1500 : best_ask() - 5;
...
void OrderBook::clean_ask() {
    ...
    if (ask_.empty()) {
        // Create a new level
        int32_t new_price = bid_.empty() ? 1500 : best_bid() + 5;
