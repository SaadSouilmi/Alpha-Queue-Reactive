...
void run_and_accumulate(const std::string& data_path, const QueueDistributions& dists,
                        const TradeIndices& trd_idx, const std::vector<LOBConfig>& lob_configs,
...
int main() {
    ...
    std::cout << "Duration: 30 minutes, Metaorder execution: 5 minutes\n";
        ...
        for (int batch_start = 0; batch_start < num_sims; batch_start += num_threads) {
            ...
            for (int i = batch_start; i < batch_end; i++) {
                futures.push_back(std::async(std::launch::async, run_and_accumulate,
                                              data_path, std::cref(dists), std::cref(trd_idx), std::cref(lob_configs),
                                              i, duration, metaorder_vol, std::ref(acc)));
            }
